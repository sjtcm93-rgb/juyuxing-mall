...
- MCP updateFunctionCode 部署不重置超时/环境变量（与 CLI 相反），优先用 MCP 部署云函数；**但部署传播有 1-3 分钟延迟，部署后立即调用会命中旧热实例，必须等 1-2 分钟再验证**
- **云开发事务头号坑（2026-09-14 提现事故根因）：db.runTransaction 回调内严禁 Promise.all 并发查询/写入——事务会话不支持并发请求，报 -501001 TransactionBusy。事务内所有操作必须逐个 await 顺序执行。已全量扫描，其余云函数的 Promise.all 都在非事务上下文，安全**
- 提现规则（2026-09-14 更新）：买家确认收货 → 佣金 frozen→settled 即可提现（customer_receive），发货超 7 天 maintenance 定时器自动收货兜底；**已取消最低提现金额**（金额>0 即可申请）；提现页显示冻结金额提示 + 全部提现按钮
- 退款 UX：B端点「同意」即自动执行；「待人工核查」单据有「重试退款」按钮（微信按 outRefundNo 幂等）；商户基本账户余额不足会拒退（新商户常见：收款次日结算到卡），需商户平台充值后重试
- 小程序版本：**1.0.7 已上传**（提现事务修复 + 无最低金额 + 地址选择模式可新增/编辑 + 冻结提示）；用户需 mp 后台选为体验版。CLI upload --project 须传项目根目录（project.config.json 在根目录）
